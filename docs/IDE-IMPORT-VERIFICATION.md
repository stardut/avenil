...
Java Application 只有 main class、缺少 classpath 时返回 `needsInput`；带可识别 Maven Spring Boot 项目的 VS Code Java launch 转换为 `mvn spring-boot:run`，并保留入口类、JVM 参数和环境变量；Java 非空 `args` 不自动转换；未知 IDEA 类型返回 `unsupported`。Import apply 只消费服务器 snapshot，不重读 source、不启动服务，并沿用现有 atomic config save 和 active/operation 保护。
