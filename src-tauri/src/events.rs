...
pub trait EventSink: Send + Sync {
    ...
    fn shutdown_error(&self, phase: &str, error: &str) {
        let _ = error;
        self.shutdown(phase);
    }
}
...
impl EventSink for TauriEventSink {
    fn runtime(&self, snapshot: &RuntimeSnapshot) {
        let _ = self.0.emit("avenil://runtime-changed", snapshot);
    }
    fn resource(&self, snapshot: &ResourceSnapshot) {
        let _ = self.0.emit("avenil://resource-changed", snapshot);
    }
    fn log(&self, chunk: &LogChunk) {
        let _ = self.0.emit("avenil://log", chunk);
    }
    fn config(&self, config: &crate::models::AppConfig) {
        let _ = self.0.emit(
            "avenil://config-changed",
            serde_json::json!({ "config": config }),
        );
    }
    fn shutdown(&self, phase: &str) {
        let _ = self.0.emit(
            "avenil://shutdown-state",
