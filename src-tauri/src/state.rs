...
impl AppState {
    pub fn new() -> Arc<Self> {
        Arc::new(Self { config: Mutex::new(AppConfig::default()), runtimes: Mutex::new(HashMap::new()), operations: Mutex::new(HashSet::new()), logs: LogHub::default(), sink: Mutex::new(None), config_path: Mutex::new(None), lifecycle: Mutex::new(()), shutting_down: AtomicBool::new(false), exit_allowed: AtomicBool::new(false), quit_requested: AtomicBool::new(false), ide_previews: Mutex::new(HashMap::new()) })
    }
    ...
    pub fn begin_shutdown(&self) -> bool {
        ...
        if self.shutting_down.compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst).is_ok() {
    ...
    pub fn abort_shutdown(&self) { let _guard = self.lifecycle.lock().unwrap(); self.exit_allowed.store(false, Ordering::SeqCst); self.shutting_down.store(false, Ordering::SeqCst); }
    pub fn allow_exit(&self) { self.exit_allowed.store(true, Ordering::SeqCst); }
    pub fn request_quit(&self) { self.quit_requested.store(true, Ordering::SeqCst); }
    pub fn clear_quit_request(&self) { self.quit_requested.store(false, Ordering::SeqCst); }
    pub fn has_quit_request(&self) -> bool { self.quit_requested.load(Ordering::SeqCst) }
}
